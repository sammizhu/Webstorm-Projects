...
class AgeVis {
    ...
    initVis() {
        ...
        // append path for area function, so that it is later behind the brush overlay
    ...
    wrangleData() {
        ...
        // Initialize votesPerAge array for ages 0 to 98 (representing ages 1 to 99)
        let votesPerAge = d3.range(0, 99).map(() => 0);

        // Iterate over each day's data and accumulate votes per age
        vis.filteredData.forEach(function(day) {
            day.ages.forEach(function(votes, age) {
                votesPerAge[age] += votes;
            });
        });
